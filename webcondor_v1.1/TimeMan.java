 ...
 * TimeMan.java
 *
 * This class updates the database according to the state of a job.
 * 
 */
package webcondor;
...
//					CompletedTime = df.format(new Date()).substring(0, 10)+" "+tokens[3];
					...
					DataRemove dr = new DataRemove(url,source_machine,destination_machine,source_DataDirectory,des_DataDirectory,FtpPutRm);
					dr.remove();
					break;
				}
...
//					stmt.executeUpdate("UPDATE "+db_table_name+" SET State='Held' WHERE id="+id+";");
	...
	/**
	 * Give the answer whether a file exists or not.
	 * @param log_url URL of the file.
	 ...
	 * @return True(succeed) of false(failed).
	 */
